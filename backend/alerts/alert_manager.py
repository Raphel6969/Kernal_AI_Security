...
class AlertManager:
    ...
    def add_webhook(
        ...
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    "INSERT INTO webhooks (id, url, is_active, created_at, trigger_safe, trigger_suspicious, trigger_malicious) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (
                        webhook_id,
                        url,
                        1,
    ...
    def get_webhooks(self) -> List[WebhookResponse]:
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    "SELECT id, url, is_active, created_at, trigger_safe, trigger_suspicious, trigger_malicious FROM webhooks"
                )
    ...
    def get_alert_history(self, limit: int = 50) -> List[AlertHistoryResponse]:
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    "SELECT id, event_id, url, status, timestamp FROM alert_history ORDER BY timestamp DESC LIMIT ?",
                    (limit,),
                )
                rows = cursor.fetchall()
                return [
                    AlertHistoryResponse(
                        id=r[0], event_id=r[1], url=r[2], status=r[3], timestamp=r[4]
                    )
                    for r in rows
                ]
    ...
    def _log_alert(self, event_id: str, url: str, status: str):
        ...
        with self._lock:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    "INSERT INTO alert_history (id, event_id, url, status, timestamp) VALUES (?, ?, ?, ?, ?)",
    ...
    async def dispatch(self, event: SecurityEvent):
        ...
        payload = {
            ...
            "content": "",  # Discord
            "embeds": [
                {  # Rich formatting for Discord
                    "title": "🚨 Aegix: Malicious Activity Detected",
                    "color": 16711680,  # Red
                    "fields": [
                        {
                            "name": "Process ID",
                            "value": str(event.execve_event.pid),
                            "inline": True,
                        },
                        {
                            "name": "Risk Score",
                            "value": f"{event.detection_result.risk_score:.1f}/100",
                            "inline": True,
                        },
                        {
                            "name": "Command",
                        ...
                        {
                            "name": "Matched Rules",
                            "value": ", ".join(event.detection_result.matched_rules)
                            or "None",
                            "inline": False,
                        },
                    ],
        ...
        # Send to all webhooks concurrently using httpx
        async def send_webhook(url: str, client: httpx.AsyncClient):
            try:
